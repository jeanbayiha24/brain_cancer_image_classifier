...
class Trainer:
    ...
    def evaluate(self):
        ...
        for inputs, labels in tqdm(self.test_dataloader, desc="Evaluating", leave=False):
            ...
            # Stocker pour métriques sklearn
...
class TFTrainer:
    ...
    def evaluate(self):
        ...
        for x_batch_test, y_batch_test in self.test_data:
            ...
            # Stocker les prédictions pour métriques sklearn
            # y_pred shape: [batch, 4], on prend argmax pour la classe prédite
            preds = tf.argmax(y_pred, axis=1)
            all_preds.extend(preds.numpy())
            all_labels.extend(y_batch_test.numpy())
        ...
        print("\nConfusion matrix (TF model):")
        print(confusion_matrix(all_labels_np, all_preds_np))

        print("\nClassification report (TF model):")
        print(classification_report(all_labels_np, all_preds_np))
